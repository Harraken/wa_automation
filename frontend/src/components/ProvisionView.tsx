...
export default function ProvisionView() {
  ...
  useEffect(() => {
    const handleProvisionUpdate = (data: any) => {
      ...
      switch (data.state) {
        ...
        case 'COMPLETING_PROFILE':
          ...
          updateStep('setup_profile', 'active', data.progress || 95);
          break;
        case 'ACTIVE':
          addLog('✅ Session WhatsApp activée avec succès !', '🎉');
          updateStep('setup_profile', 'completed', 100);
        ...
        case 'FAILED':
          ...
          addLog(`❌ Erreur: ${data.error || data.message || 'Provisioning failed'}`);
          setProgressError(data.error || data.message || 'Provisioning failed');
          setLoading(false);
          break;
      }
    };
    ...
    const handleOtpReceived = (data: any) => {
      ...
      const timestamp = new Date().toLocaleTimeString();
      const smsLogMessage = `[${timestamp}] 📱 Code SMS reçu: ${data.otp || data.code || 'Code reçu'}`;
      setLogs(prev => {
        // Avoid duplicate
        if (prev.length > 0) {
          const lastLog = prev[prev.length - 1];
          if (lastLog.includes(`Code SMS reçu: ${data.otp || data.code}`)) {
            return prev; // Skip duplicate
          }
        }
  ...
  }, [progress, startProvision, updateStep, setProgressError, complete, reset, currentSessionId]);
  ...
  const handleStartProvision = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error('❌ Failed to create provision:', err);
      setError(err.response?.data?.error || err.message || 'Failed to start provisioning');
      setLoading(false);
    }
  };
      ...
      {!progress.isActive ? (
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="max-w-2xl w-full">
            ...
            <form onSubmit={handleStartProvision} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full px-6 py-3 bg-whatsapp-green text-white rounded-lg font-medium hover:bg-whatsapp-light transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-lg"
              >
        ...
        <>
          ...
          {progress.error ? (
            <div className="bg-red-50 text-red-600 px-4 py-3 m-4 rounded-lg text-sm">
                      ...
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-whatsapp-green rounded-full flex items-center justify-center text-white font-bold mr-3 animate-pulse">
                          {activeStep.id === 'init' ? '1' : 
                           ...
                           activeStep.id === 'inject_otp' ? '7' : 
                           activeStep.id === 'setup_profile' ? '8' : '9'}
                        </div>
          ...
          <div className="flex-1 flex gap-4 p-4 overflow-hidden">
            ...
            <div className="w-64 flex-shrink-0">
              ...
              <div className="border-2 border-gray-300 rounded-lg overflow-hidden bg-gray-100 h-full flex items-center justify-center">
                {latestScreenshot ? (
                  <img 
